...
Symbolic simulation traces can be stored using the XTR format; files in this format have the extension `.xtr`. The format was introduced in UPPAAL 3.0.
...
Concrete simulation traces can be stored using the UCTR format; files in this format have the extension `.uctr`.

`.uctr`
: files are written in a JSON-like syntax, such that external editors can parse them.

`.uctr`
: file contains all transitions made in the simulation along with their delay in the `transitions` array.
